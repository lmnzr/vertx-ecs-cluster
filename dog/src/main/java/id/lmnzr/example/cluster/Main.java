    ...
    public static void main(String[] args) {
        ...
        Vertx.clusteredVertx(options, handler-> {
            if (handler.succeeded()) {
                handler.result().deployVerticle(DogVerticle.class,new DeploymentOptions().setInstances(1), deployHandler->{
                    if(handler.succeeded()){
                        log.info("Host Address "+ebOptions.getHost());
                        log.info("Cat Verticle Deployed");
                    }else{
                        log.severe("Cat Verticle Deployment Failed");
                    }
                });
            }
        });
